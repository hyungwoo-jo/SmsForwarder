...
원본 BSD-2-Clause 라이선스와 원작자 표시를 유지합니다. 소스 재배포에는 원본 저작권 표시·라이선스 조건·면책문을 보존하고, APK 재배포에는 함께 제공하는 문서 또는 자료에 이를 포함해야 합니다. AndroidX, OkHttp, Gson, XUI/XHttp/XAOP, FRPC 및 원본에서 사용하던 의존성은 각각의 원본 라이선스를 따릅니다. 본 변경은 전체 의존성 또는 네이티브 코드의 보안 감사를 의미하지 않습니다.
